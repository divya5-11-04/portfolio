...
const ProjectCard = ({ project, index, getCategoryColor }) => {
          ...
          <div className="flex gap-2 px-4 pt-3">
            {images.map((img, imgIdx) => (
              <button
                ...
                className={`relative w-12 h-9 rounded-md overflow-hidden border-2 transition-colors ${
                  activeImage === imgIdx
                    ? 'border-neon-cyan'
                    : 'border-transparent opacity-60 hover:opacity-100'
                }`}
                aria-label={`Show photo ${imgIdx + 1} of ${project.name}`}
              >
